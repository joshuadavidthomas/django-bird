...
def test_lexer_with_unclosed_tags():
    with pytest.raises(ValueError, match="Unclosed <style> tag"):
        tokenize("<style>body { color: red; }")
    with pytest.raises(ValueError, match="Unclosed <script> tag"):
        tokenize("<script>console.log('Hello');")
